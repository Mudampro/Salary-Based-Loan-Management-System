...
export async function partnerLogin(email, password) {
  ...
  const res = await partnerAxiosClient.post(
    "/partner/auth/login",
    body,
    {
      headers: {
...
export async function partnerMe() {
  const res = await partnerAxiosClient.get(
    "/partner/dashboard/me"
  );
  return res.data;
}
...
export async function getMyRemittanceAccount() {
  const res = await partnerAxiosClient.get(
    "/partner/dashboard/remittance-account"
  );
  return res.data;
}
...
export async function getMyPartnerTransactions() {
  const res = await partnerAxiosClient.get(
    "/partner/dashboard/transactions"
  );
  return res.data; 
}
